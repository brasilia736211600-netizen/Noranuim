      ...
      >
        ...
        {tab.isPaused ? (
          ...
          <NoraView
            ...
            useragent={resolveProfileUserAgent(settings$.profiles.get().find(p => p.id === (tab.profile || 'default')), true, window.electron.process.platform)}
    ...
    >
      ...
      {nIf(
        ...
        <NoraView
          ...
          scriptOnDocumentStart={composeDocumentStartScript(protectWebRtcIp && webRtcGuardScript, youTubeGuardScript, (() => { const p = settings$.profiles.get().find(p => p.id === (tab.profile || 'default')); if (p && p.timeMode === 'manual' && p.timezone) return getTimeSpoofScript(p.timezone, p.timezoneOffset || 0); return undefined; })())}
          useragent={resolveProfileUserAgent(settings$.profiles.get().find(p => p.id === (tab.profile || 'default')), tab.desktopMode || false, isIos ? 'ios' : 'android')}
          onLoad={onLoad}
          onMessage={onMessage}
          inspectable={inspectable}
          textZoom={resolvedZoom}
