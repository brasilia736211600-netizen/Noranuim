...
- 2026-09-16 follow-up fix attempt (ubuntu-latest + KVM) disproved the KVM assumption on hosted Ubuntu runners: the emulator logs `ProbeKVM: This user doesn't have permissions to use KVM (/dev/kvm)` and falls back to pure TCG software emulation (`-accel off`), booting in ~887s. That run then died instantly because the script's `set -o pipefail` is invalid under Ubuntu's `/bin/sh` (dash).
- Concluded infra fix: the runtime job now runs on `macos-15` (Apple Silicon arm64) with an arm64-v8a system image and `-PreactNativeArchitectures=arm64-v8a`, giving native Hypervisor.framework acceleration at the full core count; the script preamble was reduced to POSIX-safe `set -eu` and still waits for the package manager (`adb shell cmd package wait-for-ready`) before the connected tests.
