...
- Concluded infra fix: the runtime job runs on `ubuntu-latest` with the x86_64 system image, chmods `/dev/kvm` for the runner when present (fast path), and otherwise lets the TCG software-emulation fallback complete by raising the job timeout to 90 minutes and the emulator boot timeout to 20 minutes. The script preamble is POSIX-safe (`set -eu`) and still waits for the package manager (`adb shell cmd package wait-for-ready`) before the connected tests.
